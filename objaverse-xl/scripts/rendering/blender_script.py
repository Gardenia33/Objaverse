...
def set_camera(num_renders, i, radius, only_northern_hemisphere=False) -> bpy.types.Object:
    theta = math.acos(1 - 2 * (i + 0.5) / num_renders)  # θ
    phi = math.pi * (1 + 5 ** 0.5) * (i + 0.5)  # φ
    ...
    # camera.rotation_euler = (theta, 0, phi)
    direction = -camera.location
    rot_quat = direction.to_track_quat("-Z", "Y")
    camera.rotation_euler = rot_quat.to_euler()
